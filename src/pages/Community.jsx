...
export default function Community() {
  ...
  const translations = {
    ...
    translate: {
      button: useTranslation("Translate"),
      translating: useTranslation("Translating..."),
      original: useTranslation("Show Original"),
      error: useTranslation("Translation failed. Please try again.")
    },
    search: {
  ...
  const handleReply = async (postId) => {
    ...
    try {
      setLoading(true);
      // First get the current post to preserve author details
      ...
      // Create updated post with preserved author details and new reply
      const updatedPostWithAuthor = {
        ...updatedPost,
        author: currentPost.author, // Preserve existing post author details
        replies: updatedPost.replies.map(reply => {
          ...
          // For existing replies, try to find their author details in the current post
          const existingReply = currentPost.replies.find(r => r._id === reply._id);
          return {
            ...reply,
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col">
                      <span className="font-semibold text-gray-800">{post.author?.name || 'Unknown User'}</span>
                    ...
                    {post.replies.map(reply => (
                      ...
                      >
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div className="flex-1">
                            <div className="flex flex-col">
                              <span className="font-semibold text-gray-800">{reply.author?.name || 'Unknown User'}</span>
                              <span className="text-gray-500 text-sm">@{reply.author?.username || 'unknown'}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500 text-sm">

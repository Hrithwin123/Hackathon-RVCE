...
export const getUserDetails = async (req, res) => {
  try {
    ...
    const user = await Users.findById(userId).select('name username email createdAt');
    ...
    // Get post count
    const postCount = await Post.countDocuments({ author: userObjectId });

    // Get reply count using a more reliable aggregation
    const replyCount = await Post.aggregate([
      { $match: { 'replies.author': userObjectId } },
      { $project: { 
        replies: { 
          $filter: { 
            input: '$replies', 
            as: 'reply', 
            cond: { $eq: ['$$reply.author', userObjectId] } 
          } 
        } 
      }},
    ...
    console.log('User stats:', { userId, postCount, replyCount }); // Debug log
